...
export default function BrandCarNewsOpenAI() {
  ...
  const brands = ["", "Hyundai", "Tata Motors", "Maruti Suzuki", "Mahindra"];
  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      const r = await axios.get(
        `${API}/api/brand-car-news/stored-news?limit=${PAGE}&offset=${offset}${brandParam}&_=${Date.now()}`,
      ...
      if (r.data?.success) {
        ...
        setSp(page);
        console.log("🚗 Fetched stored brand car news:", r.data.news?.length, "articles");
      }
    } catch (e) {
  ...
  const fetchProcessed = async (page = 1) => {
    try {
      ...
      const brandParam = selectedBrand ? `&brand=${encodeURIComponent(selectedBrand)}` : '';
      const r = await axios.get(
        `${API}/api/brand-car-news/processed-news?limit=${PAGE}&offset=${offset}${brandParam}&_=${Date.now()}`,
        { headers: { "Cache-Control": "no-cache" } }
      );
      if (r.data?.success) {
        ...
        setPp(page);
        console.log("✅ Fetched processed brand car news:", r.data.news?.length, "articles");
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    console.log("🚗 Brand Car News OpenAI Component mounted");
    fetchStored(1);
    fetchProcessed(1);
  }, [selectedBrand]);
  ...
  const gen = async (id) => {
    setBusy((m) => ({ ...m, [id]: true }));
    try {
      const r = await axios.post(`${API}/api/brand-car-news/articles/${id}/generate`);
      if (!r.data?.success) throw new Error(r.data?.error || "Failed");
      await fetchStored(sp);
      await fetchProcessed(pp);
      alert("✅ Brand car article generated successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
  ...
  const manualFetch = async () => {
    setLoading(true);
    try {
      await axios.post(`${API}/api/brand-car-news/manual-fetch-news`);
      await fetchStored(1);
      setSp(1);
      alert("Brand car news fetched successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: "0 auto", fontFamily: "Inter, Arial" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <h1>🚗 Brand Car News (Hyundai, Tata, Maruti, Mahindra)</h1>
        <p>24/7 Brand-Specific Car News Fetching and OpenAI Article Generation</p>
      ...
      <div style={{ marginBottom: 16, display: "flex", gap: 10, alignItems: "center" }}>
        <label style={{ fontWeight: 600 }}>Filter by Brand:</label>
        <select 
          value={selectedBrand} 
      ...
      {tab === "stored" ? (
        <div>
          <h2>Latest Brand Car News (Page {sp} of {stp})</h2>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ flex: 1, paddingRight: 12 }}>
                      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 8 }}>
                        <span style={{ 
                          background: getBrandColor(a.brand_name), 
                          color: "white", 
                          padding: "4px 12px", 
                          borderRadius: 6, 
                          fontSize: 12, 
                      ...
                      <div style={{ display: "flex", gap: 14, color: "#666", fontSize: 12, flexWrap: "wrap" }}>
                        ...
                        {a.openai_processed ? <span style={{ color: "#28a745", fontWeight: 700 }}>✅ Processed</span> : null}
                      </div>
                      {a.description && (
                        <div style={{ marginTop: 8, color: "#555", fontSize: 13, lineHeight: 1.5 }}>
                    ...
                    <button
                      onClick={() => gen(a.id)}
                      disabled={isBusy}
                      style={{ padding: "10px 14px", background: isBusy ? "#95a5a6" : "#00b894", color: "#fff", border: "none", borderRadius: 8, whiteSpace: "nowrap" }}
                    >
        ...
        <div>
          <h2>✅ Processed Brand Car Articles (Page {pp} of {ptp})</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {processed.map((a) => {
              ...
              return (
                <div key={a.id} style={{ border: "1px solid #a7f3d0", borderRadius: 10, padding: 16, background: "#f0fff4" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                    <button onClick={() => navigator.clipboard.writeText(a.openai_ready_article || "")}>📋 Copy HTML</button>
                    <button onClick={() => {
