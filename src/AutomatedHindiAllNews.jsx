...
export default function AutomatedHindiAllNews() {
  ...
  const fmt = (v) => {
    ...
    if (timeMatch) {
      ...
      return { date: new Date(v).toLocaleDateString("en-IN"), time: formattedTime, };
    }
    return { date: "-", time: "-" };
  };
  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      const r = await axios.get(
        `${API}/api/hindi-all-news/stored-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
      ...
      if (r.data?.success) {
        ...
        setSp(page);
        console.log("📰 Fetched stored Hindi all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
  ...
  const fetchProcessed = async (page = 1) => {
    try {
      const offset = (page - 1) * PAGE;
      const r = await axios.get(
        `${API}/api/hindi-all-news/processed-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
        { headers: { "Cache-Control": "no-cache" } }
      );
      if (r.data?.success) {
        ...
        setPp(page);
        console.log("✅ Fetched processed Hindi all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    console.log("🔧 Hindi All News Component mounted");
  ...
  const gen = async (id) => {
    setBusy((m) => ({ ...m, [id]: true }));
    try {
      const r = await axios.post(`${API}/api/hindi-all-news/articles/${id}/generate`);
      if (!r.data?.success) throw new Error(r.data?.error || "Failed");
      await fetchStored(sp);
      await fetchProcessed(pp);
      alert("✅ Hindi all news article generated successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
  ...
  const manualFetch = async () => {
    setLoading(true);
    try {
      await axios.post(`${API}/api/hindi-all-news/manual-fetch-news`);
      await fetchStored(1);
      setSp(1);
      alert("Hindi all news fetched successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1200, margin: "0 auto", fontFamily: "Inter, Arial" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <h1>📰 हिंदी ऑल न्यूज़ ऑटोमेशन</h1>
        <p>24/7 Hindi All News Fetching & One-Click Article Generation</p>
      ...
      <div style={{ display: "flex", gap: 12, justifyContent: "center", marginBottom: 14 }}>
        ...
        <button 
          ...
          onClick={manualFetch}
        >
          {loading ? "⏳ Fetching…" : "📰 Fetch New Hindi All News"}
        </button>
      </div>

      {tab === "stored" ? (
        <div>
          <h2>Latest Stored Hindi All News (Page {sp} of {stp})</h2>
            ...
            {stored.map((a) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isBusy ? "⏳ Generating…" : "🧠 Generate Hindi Article (AI)"}
                  </button>
                </div>
              );
            })}
        ...
        <div>
          <h2>✅ Processed Hindi Articles (Page {pp} of {ptp})</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {processed.map((a) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                    <button onClick={() => navigator.clipboard.writeText(a.ready_article || "")}>
                      📋 Copy Full HTML
                    </button>
                    <button onClick={() => {
...
function Pager({ page, total, totalCount, onChange }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: 16 }}>
      ...
      {nums.map((n) => (
        <button key={n} onClick={() => onChange(n)} style={{ fontWeight: n===page ? "bold" : "normal" }}>
          {n}
        </button>
      ))}
