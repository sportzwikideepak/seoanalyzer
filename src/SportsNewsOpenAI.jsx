...
export default function SportsNewsOpenAI() {
  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      const r = await axios.get(
        `${API}/api/sports-openai/stored-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
      ...
      if (r.data?.success) {
        ...
        setSp(page);
        console.log("⚽ Fetched stored sports news:", r.data.news?.length, "articles");
      }
    } catch (e) {
  ...
  const fetchProcessed = async (page = 1) => {
    try {
      const offset = (page - 1) * PAGE;
      const r = await axios.get(
        `${API}/api/sports-openai/processed-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
        { headers: { "Cache-Control": "no-cache" } }
      );
      if (r.data?.success) {
        ...
        setPp(page);
        console.log("✅ Fetched processed sports news:", r.data.news?.length, "articles");
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    console.log("⚽ Sports News OpenAI Component mounted");
  ...
  const gen = async (id) => {
    setBusy((m) => ({ ...m, [id]: true }));
    try {
      const r = await axios.post(`${API}/api/sports-openai/articles/${id}/generate`);
      if (!r.data?.success) throw new Error(r.data?.error || "Failed");
      await fetchStored(sp);
      await fetchProcessed(pp);
      alert("✅ Sports article generated successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
  ...
  const manualFetch = async () => {
    setLoading(true);
    try {
      await axios.post(`${API}/api/sports-openai/manual-fetch-news`);
      await fetchStored(1);
      setSp(1);
      alert("Sports news fetched successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1200, margin: "0 auto", fontFamily: "Inter, Arial" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <h1>⚽ Sports News OpenAI</h1>
        <p>24/7 Sports News Fetching and OpenAI One-Click Article Generation</p>
      ...
      {tab === "stored" ? (
        <div>
          <h2>Latest Sports News (Page {sp} of {stp})</h2>
        ...
        <div>
          <h2>✅ OpenAI Processed Sports Articles (Page {pp} of {ptp})</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {processed.map((a) => {
