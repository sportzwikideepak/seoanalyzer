...
export default function AutomatedAllNews() {
  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      const r = await axios.get(
        `${API}/api/all/stored-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
      ...
      if (r.data?.success) {
        ...
        setSp(page);
        console.log("📰 Fetched stored all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
  ...
  const fetchProcessed = async (page = 1) => {
    try {
      const offset = (page - 1) * PAGE;
      const r = await axios.get(
        `${API}/api/all/processed-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
        { headers: { "Cache-Control": "no-cache" } }
      );
      if (r.data?.success) {
        ...
        setPp(page);
        console.log("✅ Fetched processed all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const gen = async (id) => {
    setBusy((m) => ({ ...m, [id]: true }));
    try {
      const r = await axios.post(`${API}/api/all/articles/${id}/generate`);
  ...
  const manualFetch = async () => {
    setLoading(true);
    try {
      await axios.post(`${API}/api/all/manual-fetch-news`);
      await fetchStored(1);
      setSp(1);
      alert("News fetched.");
    } catch (e) {
