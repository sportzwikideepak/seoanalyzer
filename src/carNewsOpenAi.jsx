  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      if (r.data?.success) {
        ...
        console.log("🚗 Fetched stored car news:", newsData.length, "articles");

        console.log("📊 Total count:", total);

        if (newsData.length === 0) {

          console.warn("⚠️ No news data found in response");

        }

      } else {

        console.error("❌ API response not successful:", r.data);
      ...
      console.error("❌ Error fetching stored news:", e);
  ...
  const manualFetch = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        console.log("🔄 Refreshing data after manual fetch...");
