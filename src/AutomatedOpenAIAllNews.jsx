...
export default function AutomatedOpenAIAllNews() {
  ...
  const fetchStored = async (page = 1) => {
    ...
    try {
      ...
      const r = await axios.get(
        `${API}/api/openai-all/stored-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
      ...
      if (r.data?.success) {
        ...
        setSp(page);
        console.log("📰 Fetched stored OpenAI all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
  ...
  const fetchProcessed = async (page = 1) => {
    try {
      const offset = (page - 1) * PAGE;
      const r = await axios.get(
        `${API}/api/openai-all/processed-news?limit=${PAGE}&offset=${offset}&_=${Date.now()}`,
        { headers: { "Cache-Control": "no-cache" } }
      );
      if (r.data?.success) {
        ...
        setPp(page);
        console.log("✅ Fetched processed OpenAI all news:", r.data.news?.length, "articles");
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    console.log("🤖 OpenAI All News Component mounted");
  ...
  const gen = async (id) => {
    setBusy((m) => ({ ...m, [id]: true }));
    try {
      const r = await axios.post(`${API}/api/openai-all/articles/${id}/generate`);
      if (!r.data?.success) throw new Error(r.data?.error || "Failed");
      await fetchStored(sp);
      await fetchProcessed(pp);
      alert("✅ OpenAI article generated successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
  ...
  const manualFetch = async () => {
    setLoading(true);
    try {
      await axios.post(`${API}/api/openai-all/manual-fetch-news`);
      await fetchStored(1);
      setSp(1);
      alert("OpenAI news fetched successfully!");
    } catch (e) {
      alert(e.response?.data?.error || e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1200, margin: "0 auto", fontFamily: "Inter, Arial" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <h1>🤖 OpenAI All News</h1>
        <p>24/7 News Fetching & One-Click Article Generation with OpenAI</p>
      ...
      {tab === "stored" ? (
        ...
        <div>
          <h2>✅ OpenAI Processed Articles (Page {pp} of {ptp})</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {processed.map((a) => {
